...
def login_by_cookie(browser_manager: SeleniumBrowserManager, config: Dict[str, Any]) -> bool:
    """
    使用Cookie登录
    ...
    try:
        ...
        cookie_path = login_config.get('cookie_path')
        website_config = config.get('WEBSITE', {})
        url = website_config.get('url')

        if not cookie_path:
            logger.error("未配置cookie文件路径")
            return False
        ...
        # 加载cookies
        if not browser_manager.load_cookies(cookie_path):
        ...
        # 验证登录状态
        if browser_manager.verify_login_status():
            logger.info("Cookie登录成功")
            return True
        else:
            logger.warning("Cookie登录失败")
            return False

    except Exception as e:
        logger.error(f"Cookie登录时出错: {e}")
        return False


def login_by_form(browser_manager: SeleniumBrowserManager, config: Dict[str, Any]) -> bool:
    """
    使用表单登录
    ...
    try:
        ...
        login_elements = elements.get('login', {})
        user = config.get('USER', {})
        # 导航到登录页面
        browser_manager.navigate_to("https://www.nodeseek.com/signIn.html")

        # 等待页面加载完成
        time.sleep(3)

        # 填写用户名
        username_input = login_elements.get('username_input')
        if username_input:
            if not browser_manager.fill_input(username_input, user.get('username', '')):
                logger.error("填写用户名失败")
                return False

        # 填写密码
        password_input = login_elements.get('password_input')
        if password_input:
            if not browser_manager.fill_input(password_input, user.get('password', '')):
                logger.error("填写密码失败")
                return False

        # 点击提交按钮
        submit_button = login_elements.get('submit_button')
        if submit_button and not browser_manager.click_element(submit_button):
        ...
        # 等待Cloudflare验证
        token = None
        if browser_manager.is_element_present({'type': 'name', 'value': 'cf-turnstile-response'}, wait_time=3):
            logger.info("等待Cloudflare验证...")
            token = browser_manager.solve_turnstile("0x4AAAAAAAaNy7leGjewpVyR",
                                                    "https://www.nodeseek.com/signIn.html", )
            print(token)
            # 注入token
            browser_manager.inject_token(token)
            time.sleep(2)
        ...
        """
        browser_manager.driver.execute_script(script)
        logger.info(f"已模拟Turnstile回调并发送登录请求，token: {token}")
        # 验证登录状态  查找class = 'user-card'的元素
        if browser_manager.is_element_present({'type': 'class', 'value': 'user-card'}, wait_time=3):
            logger.info("表单登录成功")

            # 如果配置了保存cookie
            login_config = config.get('LOGIN', {})
            if login_config.get('save_cookie'):
                cookie_path = login_config.get('cookie_path')
                if cookie_path:
                    browser_manager.save_cookies(cookie_path)
        ...
        logger.error(f"表单登录时出错: {e}")
        return False


def login(browser_manager: SeleniumBrowserManager, config: Dict[str, Any]) -> bool:
    """
    执行登录操作
    ...
    try:
        # 获取登录配置
        login_config = config.get('LOGIN', {})
        login_method = login_config.get('method', 'form')

        logger.info(f"使用{login_method}方式登录")

        # 根据配置选择登录方式
        if login_method == 'cookie':
            success = login_by_cookie(browser_manager, config)
        else:
            success = login_by_form(browser_manager, config)
...
def notify(success: bool, message: str, screenshot_path: Optional[str], config: Dict[str, Any]) -> None:
    """
    发送通知
    ...
    #
    # try:
    #     website_name = config.get('WEBSITE', {}).get('name', '未知网站')
    ...
    #     notifier.send_signin_result(success, website_name, message, screenshot_path)
    #     logger.info("通知已发送")
    #
    # except Exception as e:
